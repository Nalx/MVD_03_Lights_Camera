...
void Game::init() {
    ...
    graphics_system_.getMaterial(default_mat_id).diffuse_map = Parsers::parseTexture("data/assets/white.tga");
   ...
   /* int ent_teapot = ECS.createEntity("Teapot");
	...
	ECS.getComponentFromEntity<Transform>(ent_cube).translate(0, -0.6, 1.5);
	ECS.getComponentFromEntity<Transform>(ent_cube).rotateLocal(-0.4f, lm::vec3(0, 1, 0));
	...
	light_1_comp.color = lm::vec3(0.0f,0.0f,0.0f);
